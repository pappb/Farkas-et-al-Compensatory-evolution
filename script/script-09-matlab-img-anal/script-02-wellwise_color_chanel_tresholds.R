# Ez a script segít a matlab scriptnek.
# A Matlab script elso menete general egy nagy csv filet, amiben a kepenbkent megallapitott tresholdok vannak.
# egy well-ben sok kep van. Wellenként mediant szamol a tresholdokbol, es kiirja egy masik tablazatba.
# A Matlab script masodik menete ezt a tablazatot hasznalja fel amikor 8 bites jpg kepeket csinal a 16 bites szinmelysegu raw fileokbol.
#
# Menet kozben general nehany abrat, ami mutatja hogy az egyes kepekbol szamolt trasholdok mennyire szornak
...
dir0<-"/home/feketeg/mounted-shares/raid-of-group/PROJECTS/YeastMorphology-2015-FarkasZoli_KovacsKaroly/DATA_of_microscope/02-Operetta_Images-2021_06_30-newMatlab_Script-wellwiseColorTresholds-reconstructed-naturals-epistasis/"
...
# 
# plate_layout_tbl<-read_csv("data/preprocess_calmorph_output/YMP_tables/YMP_plate_layout.csv")
# 
# #######x


tbl1<-read_csv(file = paste0(dir0,"treshold_tables_based_on_control_well_images.csv"))
...
ncol=1)
ggsave(filename = paste0(dir0,"plate-treshold-wellwise.pdf"), width = 1.5*19, height = 1.5*9)
...
tbl2<- tbl1 %>%
  select(-wvp) %>%
  group_by(YMP_folder_name,short_folder_name, wellName) %>% summarise_all(median)



cowplot::plot_grid(
  tbl2 %>%  ggplot(mapping = aes(y=treshold_high_conA,x=wellName)) +
    geom_point(data = tbl1, mapping = aes(), color="palevioletred3")+
    geom_point()+
    facet_grid(.~short_folder_name)+
    theme(axis.text.x = element_text(angle=90), axis.title.x = element_blank())+
    theme(legend.position = "none"),
  tbl2 %>%  ggplot(mapping = aes(y=treshold_low_conA,x=wellName)) +
    geom_point(data = tbl1, mapping = aes(), color="palevioletred3")+
    geom_point()+
    facet_grid(.~short_folder_name)+
    theme(axis.text.x = element_text(angle=90), axis.title.x = element_blank())+
    theme(legend.position = "none"),
  tbl2 %>%  ggplot(mapping = aes(y=treshold_high_dapi,x=wellName)) +
    geom_point(data = tbl1, mapping = aes(), color="palevioletred3")+
    geom_point()+
    facet_grid(.~short_folder_name)+
    coord_cartesian(ylim=c(0,max(tbl2$treshold_high_dapi)*1.1))+
    theme(axis.text.x = element_text(angle=90), axis.title.x = element_blank())+
    theme(legend.position = "none"),
  tbl2 %>%  ggplot(mapping = aes(y=treshold_low_dapi,x=wellName)) +
    geom_point(data = tbl1, mapping = aes(), color="palevioletred3")+
    geom_point()+
    facet_grid(.~short_folder_name)+
      coord_cartesian(ylim=c(0,max(tbl2$treshold_high_dapi)*1.1))+
  ...
  ncol=1)
ggsave(filename = paste0(dir0,"aggregated-plate-treshold-wellwise.pdf"), width = 1.5*19, height = 1.5*9)
...
write_csv(tbl2,paste0(dir0,"aggregated-plate-treshold-wellwise.csv"))
write_tsv(tbl2,paste0(dir0,"aggregated-plate-treshold-wellwise.tsv"))
